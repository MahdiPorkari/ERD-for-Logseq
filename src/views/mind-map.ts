...
function branchBoxSize(b: TreeNode): { w: number; h: number } {
  const w = 180 + Math.min(b.children.length, 5) * 5;
  const h = measureBoxHeight(b.name, w, 12, 600, 44);
  return { w, h };
}

/** Row height = max of branch box and total leaf stack */
function rowHeight(b: TreeNode): number {
  const { h: brH } = branchBoxSize(b);
  if (!b.children.length) return brH;
  const leafHeights = b.children.map((k) => measureBoxHeight(k.name, LF_W, 12, 400, 32));
...
function sideHeight(list: TreeNode[], gap: number): number {
  return list.reduce((s, b) => s + rowHeight(b), 0) + (list.length - 1) * gap;
}
...
export function layoutMindMap(root: TreeNode, _maxDepth: number): LayoutResult {
  ...
  function renderSide(list: TreeNode[], isRight: boolean, startIndex: number): void {
    ...
    list.forEach((b, i) => {
      ...
      const lfX = isRight ? brX + brW + lfColGap : brX - lfColGap - LF_W;
      ...
      if (b.children.length) {
        ...
        b.children.forEach((k, ki) => {
          ...
          const ex2 = isRight ? lfX : lfX + LF_W;
  ...
  const leftEdge = rootCx - rootW / 2 - colGap - 200 - lfColGap - LF_W;
  const rightEdge = rootCx + rootW / 2 + colGap + 200 + lfColGap + LF_W;
  return {
    elements: els,
